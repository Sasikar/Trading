...
class MarketWidget : AppWidgetProvider() {
    companion object {
        ...
        private fun refreshIntent(context: Context): PendingIntent {
            ...
            return PendingIntent.getBroadcast(context, 1001, intent, PendingIntent.FLAG_UPDATE_CURRENT or PendingIntent.FLAG_IMMUTABLE)
        }
    ...
    override fun onUpdate(context: Context, manager: AppWidgetManager, ids: IntArray) { ids.forEach { refreshWidget(context, manager, it) } }
    override fun onReceive(context: Context, intent: Intent) {
        super.onReceive(context, intent)
        if (intent.action == ACTION_REFRESH) {
            val manager = AppWidgetManager.getInstance(context)
            manager.getAppWidgetIds(android.content.ComponentName(context, MarketWidget::class.java)).forEach { refreshWidget(context, manager, it) }
        }
    }
    ...
    private fun fetchPrice(symbol: String): String? = try {
        ...
        when { price >= 1000 -> String.format(Locale.US, "$%,.0f", price); price >= 1 -> String.format(Locale.US, "$%.2f", price); else -> String.format(Locale.US, "$%.6f", price).trimEnd('0').trimEnd('.') }
    ...
    private fun get(urlString: String): String {
        ...
        connection.connectTimeout = 8000; connection.readTimeout = 8000; connection.requestMethod = "GET"
