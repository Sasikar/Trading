...
class MarketWidget : AppWidgetProvider() {
    ...
    override fun onUpdate(context: Context, manager: AppWidgetManager, ids: IntArray) {
        ids.forEach { updateFromCache(context, manager, it) }
        ids.forEach { refreshInBackground(context, manager, it) }
    }
    ...
    private fun updateFromCache(context: Context, manager: AppWidgetManager, id: Int) {
        try {
            ...
            // Never allow a widget update exception to take down the provider.
        }
    }

    private fun refreshInBackground(context: Context, manager: AppWidgetManager, id: Int) {
        Thread {
            try {
                val prices = fetchPrices()
                val fomo = fetchFomo()
                if (prices.isEmpty() && fomo == null) return@Thread
                ...
                // Network/provider failures must not make the launcher show an error widget.
            }
        }.start()
    }
    ...
    private fun get(urlString: String): String {
        ...
        connection.setRequestProperty("User-Agent", "Mozilla/5.0 (Android) MemeWidget/1.2")
        return try {
