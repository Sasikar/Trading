...
export default {
  async fetch(request: Request, env: Env): Promise<Response> {
    ...
    if (url.pathname === "/complete-login") return session.fetch(new Request(`${url.origin}/session/complete-login`, { headers: { "x-login-token": request.headers.get("x-login-token") || "" }));
    if (url.pathname === "/sync") return session.fetch(new Request(`${url.origin}/session/sync`, { headers: { "x-login-token": request.headers.get("x-login-token") || "" }));
    if (url.pathname === "/transactions") {
      const expected = await session.storage.get<string>(LOGIN_TOKEN_KEY).catch(() => undefined);
      if (!expected || request.headers.get("x-login-token") !== expected) return json({ status: "unauthorized" }, 401);
      const rows = await env.DB.prepare("SELECT id, timestamp, raw_json, synced_at FROM koinly_transactions ORDER BY timestamp DESC LIMIT 250").all();
...
export class BrowserSession extends DurableObject<Env> {
  ...
  private async startManualLogin(): Promise<Response> {
    ...
    return json({ status: "login_required", login_token: loginToken, message: "Open live_view_url, sign in to Koinly with Google, then tap Complete Login.", live_view_url: devtoolsFrontendUrl });
  }
  ...
  private async completeManualLogin(request: Request): Promise<Response> {
    ...
    } catch (error) { return json({ status: "login_session_error", message: error instanceof Error ? error.message : String(error) }, 500); }
  }
  ...
  private async syncTransactions(request: Request): Promise<Response> {
    ...
    try {
      ...
      if (page.url().includes("/login")) {
        const cdp = await page.createCDPSession();
        const { devtoolsFrontendUrl } = await cdp.send("Cloudflare.getLiveView", { mode: "tab", expiresInMs: 3600000 });
        await this.ctx.storage.put(BROWSER_KEY, browser.sessionId()); await browser.disconnect();
      ...
      if (!rows.length) { await browser.disconnect(); return json({ status: "needs_selector_check", message: "Koinly loaded, but no transaction rows were detected. The authenticated DOM needs one selector update." }, 422); }
      ...
      await this.ctx.storage.put(SESSION_KEY, JSON.stringify(await page.context().storageState()));
      await browser.close(); this.browser = undefined;
      return json({ status: "ok", count: rows.length, synced_at: syncedAt });
    } catch (error) { return json({ status: "browser_session_error", message: error instanceof Error ? error.message : String(error) }, 500); }
  }

  private async validToken(request: Request) { const expected = await this.ctx.storage.get<string>(LOGIN_TOKEN_KEY); return !!expected && request.headers.get("x-login-token") === expected; }
  private async getFreshBrowser() { this.browser = await launch(this.env.BROWSER, { keep_alive: 600000 }); const page = await this.browser.newPage(); return { browser: this.browser, page }; }
  private async getBrowserWithState(storageState: unknown) { this.browser = await launch(this.env.BROWSER, { keep_alive: 600000 }); const context = await this.browser.newContext({ storageState }); const page = await context.newPage(); return { browser: this.browser, page }; }
}

async function sha256(input: string) { const digest = await crypto.subtle.digest("SHA-256", new TextEncoder().encode(input)); return [...new Uint8Array(digest)].map(b => b.toString(16).padStart(2, "0")).join(""); }
...
function json(value: unknown, status = 200) { return new Response(JSON.stringify(value, null, 2), { status, headers: { "content-type": "application/json; charset=utf-8", ...cors() } }); }
