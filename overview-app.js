...
async function loadMarket(){
  ...
  try{
    ...
    if(btc){set('btc-price', money(btc.price)); set('btc-chg', (btc.pct>=0?'+':'')+btc.pct.toFixed(2)+'%', btc.pct>=0?'#62e3a0':'#ff6f7c'); any=true;}
    else set('btc-price','—');
    if(eth){set('eth-price', money(eth.price)); set('eth-chg', (eth.pct>=0?'+':'')+eth.pct.toFixed(2)+'%', eth.pct>=0?'#62e3a0':'#ff6f7c'); any=true;}
    else set('eth-price','—');
    if($('mkt-source')) $('mkt-source').textContent=src||'OFFLINE';
  ...
  try{
    ...
    if(snap&&snap.price!=null){
      ...
      if(pct!=null) set('ndx-chg', (pct>=0?'+':'')+(+pct).toFixed(2)+'%', pct>=0?'#62e3a0':'#ff6f7c');
  ...
  try{
    ...
    if(row){
      ...
      set('fng-val', String(v));
      set('fng-lab', lab, v>=55?'#62e3a0':(v<=45?'#ff6f7c':'#e6c878'));
      any=true;
    } else {
      set('fng-val','—'); set('fng-lab','Unavailable');
    }
  }catch(e){console.warn('fng',e); set('fng-val','—'); set('fng-lab','Unavailable');}

  if($('mkt-status')) $('mkt-status').textContent=any?'LIVE / SNAPSHOT':'OFFLINE';
}

async function fetchKlines(interval,limit){
  /* Browser-direct Kraken OHLC only. Never use 21600 as 1M. */
  const iv={ '4h':240,'1d':1440,'1w':10080,'1h':60 }[interval];
  if(!iv) throw new Error('unsupported interval '+interval+' on Pages (use static monthly for Macro)');
...
if(!book||!book.bids||!book.asks||!book.bids.length){statusEl.className='struct-status unav';statusEl.textContent='⚪ Unavailable';subEl.textContent='Orderbook unavailable on static Pages.';set('ms-spread','Data Unavailable');set('ms-bid','Data Unavailable');set('ms-conc','Data Unavailable');set('ms-fresh','Data Unavailable');return;}
